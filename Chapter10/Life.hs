...
glider = [(4,2),(2,3),(4,3),(3,4),(4,4)] :: Board
...
neighbors (x,y) = map wrap [(x-1,y-1), (x,y-1), (x+1,y-1), (x-1,y),
                  ...
                  where wrap (x,y) = (((x-1) `mod` width) + 1,
                                      ((y-1) `mod` height) + 1)
...
births :: Board -> [Pos]
births board = [pos | pos <- rmDups (concatMap neighbors board),
                             isEmpty board pos, liveNeighbors board pos == 3]
               where rmDups []     = []
