...
-- Is this move valid?
-- E.g. valid initial 1 3 == True                   <- The 1st row has 5 stars
-- E.g. valid initial 4 3 == False                  <- The 4th row only has 2 stars
valid :: Board -> Int -> Int -> Bool
valid board row num = board !! (row - 1) >= num

-- Return a new board showing the results of a valid move on a board
-- E.g. move initial 1 3 == [2,4,3,2,1]             <- Remove 3 stars from the 1st row
...
-- E.g. putBoard initial => displays all five rows (initial == [5,4,3,2,1])
putBoard :: Board -> IO ()
...
putBoard' :: Board -> IO ()
putBoard' xs = go xs 1 where go []       _ = return ()
